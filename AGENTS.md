...
- Deal tags (enrichment):
  ...
  - **Position in `TAG_VOCABULARY` is a resource, and it is close to zero-sum.** v13 moved `entertainment` from seventeenth to seventh, changing nothing else: its own classes went +4 (tickets 2/5 → 5/5, books 3/5 → 4/5) and it cost 2 elsewhere. Spend the early slots on categories that are losing deals they should win, and expect whatever gets passed to give some up.
